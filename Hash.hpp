...
namespace sha
{
  ...
  public:
    Type Rot(Type v, int n) { return (v << n) | (v >> (32 - n)); }
    Type Shr(Type v, int n) { return (v >> n); }
    Type Sigma0(Type x) { return Rot(x, 30) ^ Rot(x, 19) ^ Rot(x, 10); }
    Type Sigma1(Type x) { return Rot(x, 26) ^ Rot(x, 21) ^ Rot(x, 7); }
    Type sigma0(Type x) { return Rot(x, 25) ^ Rot(x, 14) ^ Shr(x, 3); }
    Type sigma1(Type x) { return Rot(x, 15) ^ Rot(x, 13) ^ Shr(x, 10); }
    Type Ch(Type x, Type y, Type z) { return (x & y) ^ (~x & z); }
    Type Maj(Type x, Type y, Type z) { return (x & y) ^ (y & z) ^ (x & z); }
    auto padding(const std::string &input)
    {
      size_t size = input.size();
      size_t padding_size =
          ((size + 9 + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
    ...
    auto operator()()
    {
      ...
      for (int num = 0; num < size; num += 64)
      {
        ...
        for (int t = 0; t < 64; ++t)
        {
          if (t < 16)
          {
            T1 = X[t] = loadMessge(messages, num + t * 4);
          }
          else
          {
  ...
  public:
    ...
    static constexpr std::array<Type, 80> K = {
  ...
  public:
    Type Rot(Type v, int n) { return (v >> n) | (v << (64 - n)); }
    Type Shr(Type v, int n) { return (v >> n); }
    Type Sigma0(Type x) { return Rot(x, 28) ^ Rot(x, 34) ^ Rot(x, 39); }
    Type Sigma1(Type x) { return Rot(x, 14) ^ Rot(x, 18) ^ Rot(x, 41); }
    Type sigma0(Type x) { return Rot(x, 1) ^ Rot(x, 8) ^ Shr(x, 7); }
    Type sigma1(Type x) { return Rot(x, 19) ^ Rot(x, 61) ^ Shr(x, 6); }
    Type Ch(Type x, Type y, Type z) { return (x & y) ^ (~x & z); }
    Type Maj(Type x, Type y, Type z) { return (x & y) ^ (y & z) ^ (x & z); }
    auto padding(const std::string &input)
    {
      size_t size = input.size();
      size_t padding_size =
          ((size + 17 + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
      std::vector<uint8_t> ret(padding_size);
      for (int i = 0; i < size; i++)
      {
        ret[i] = static_cast<uint8_t>(input[i]);
    ...
    auto operator()()
    {
      ...
      for (int num = 0; num < size; num += 128)
      {
        ...
        for (int t = 0; t < 80; ++t)
        {
          if (t < 16)
          {
            T1 = X[t] = loadMessge(messages, num + t * 8);
          }
          else
          {
            T1 = X[t & 0xf] += sigma0(X[(t + 1) & 0x0f]) +
        ...
        {
          H[i] += arr[i];
          std::cout << std::setfill('0') << std::setw(16) << std::hex << arr[i]
                    << std::endl;
        }
      }
      return std::make_pair(seed, H);
